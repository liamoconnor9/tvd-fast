...
if True:
    loaded_NW_1 = np.load('/home/x-loconnor/tvd-fast/ftle/quadrants/lya_NW_40_0040.npz')
    N = np.shape(loaded_NW_1['lya'])[0]
    dt = 1e-2
    times = np.array([i*dt for i in range(N)])
    reshaped_NW_1 = np.reshape(loaded_NW_1['lya'], (N, 256, 64))

    loaded_SW_1 = np.load('/home/x-loconnor/tvd-fast/ftle/quadrants/lya_SW_40_0040.npz')
    reshaped_SW_1 = np.reshape(loaded_SW_1['lya'], (N, 256, 64))

    loaded_SE_1 = np.load('/home/x-loconnor/tvd-fast/ftle/quadrants/lya_SE_40_0040.npz')
    reshaped_SE_1 = np.reshape(loaded_SE_1['lya'], (N, 256, 64))

    loaded_NE_1 = np.load('/home/x-loconnor/tvd-fast/ftle/quadrants/lya_NE_40_0040.npz')
    ...
    loaded_NW_33 = np.load('/home/x-loconnor/tvd-fast/ftle/quadrants/lya_NW_72_0072.npz')
    N = np.shape(loaded_NW_33['lya'])[0]
    dt = 1e-2
    times = np.array([i*dt for i in range(N)])
    reshaped_NW_33 = np.reshape(loaded_NW_33['lya'], (N, 256, 64))

    loaded_SW_33 = np.load('/home/x-loconnor/tvd-fast/ftle/quadrants/lya_SW_72_0072.npz')
    reshaped_SW_33 = np.reshape(loaded_SW_33['lya'], (N, 256, 64))

    loaded_SE_33 = np.load('/home/x-loconnor/tvd-fast/ftle/quadrants/lya_SE_72_0072.npz')
    reshaped_SE_33 = np.reshape(loaded_SE_33['lya'], (N, 256, 64))

    loaded_NE_33 = np.load('/home/x-loconnor/tvd-fast/ftle/quadrants/lya_NE_72_0072.npz')
...
roll_index = round((1050 / 2048) * 512)
ftle1 = np.roll(ftle1, shift=roll_index, axis=0)
ftle2 = np.roll(ftle2, shift=roll_index, axis=0)
