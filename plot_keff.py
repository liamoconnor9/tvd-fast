...
mew=0.5
series = [
    ("keff_0p18.csv", 0.18, dict(marker="s", linestyle="none", label=r"$k_y=0.18$", ms=ms, color="lime", alpha=alpha, mec="black", mew=mew)),
    ("keff_0p29.csv", 0.29, dict(marker="o", linestyle="none", label=r"$k_y=0.29$", ms=ms, color="magenta", alpha=alpha, mec="black", mew=mew)),
    ("keff_1p0.csv",  1.00, dict(marker="^", linestyle="none", label=r"$k_y=1.0$",  ms=ms, color="cyan", alpha=alpha, mec="black", mew=mew)),
]
...
plt.ylabel(r"$k_{\mathrm{eff}}^{2}\, / \, \mathrm{Rm}$")
...
color_handles = [
    Line2D([0], [0], marker='^', color='none', markerfacecolor='black', markersize=8, label=r"$n=1$"),
    Line2D([0], [0], marker='o', color='none', markerfacecolor='black', markersize=8, label=r"$n=2$"),
    Line2D([0], [0], marker='*', color='none', markerfacecolor='black', markersize=10, label=r"other"),
...
# plt.legend(handles=color_handles, frameon=False, ncol=2)
plt.legend(handles=color_handles, frameon=True, ncol=2, handletextpad=0.3)

# plt.legend(handles=color_handles, title="Color = Category", frameon=False)

plt.tight_layout()
figname = "keff_plot.png"
plt.savefig(figname, dpi=300)
print(figname)
figname = "keff_plot.pdf"
plt.savefig(figname, dpi=300)
print(figname)
# plt.show()
