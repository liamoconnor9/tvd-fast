...
def plot_spectrum(dir):
    ...
    plt.axvline(2/23.33, linestyle='dashed', color='yellow')
    ...
    if 'floquet' in dir:
        ...
        cutoff = 12
        plt.scatter(xf[1:cutoff], 2.0/N * np.abs(yf[1:cutoff]), color='white', alpha=1.0, zorder=10)
        ...
        # plt.plot(xf, 2.0/N * np.abs(yf[0:N//2]), color='yellow')
        ...
        plt.plot(xf, 2.0/N * np.abs(yf[0:N//2]), color='white', alpha=0.8)
        plt.xscale('log')
        # if '1e3' in dir:
