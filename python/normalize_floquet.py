...
for index in range(1, 2501):
    logger.info("reading checkpoint: {}".format(index))
    load_cp = '{}/checkpoint/checkpoint_s{}.h5'.format(suffix, index)
    load_path = "{}/{}".format(path, load_cp)
    with h5py.File(load_path, "r") as file:
        A.load_from_global_grid_data(file['tasks']['A'][()][0, ...])
        total += integ(b@b)
        total = total.evaluate()
total = (total / Ly / Lz / Lx / 2500).evaluate()

if CW.rank == 0:
    print("mean b@b: {}".format(total['g']))
    print("sqrt(mean b@b: {})".format(np.sqrt(total['g'])))
