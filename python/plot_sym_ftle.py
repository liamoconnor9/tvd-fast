...
ftle1_quad = np.zeros((256, 64))
ftle2_quad = np.zeros((256, 64))

if True:

    loaded_SW_1 = np.load('/home/x-loconnor/mhd/d2/quadrants/lya_SW_1_0001.npz')
    N = np.shape(loaded_SW_1['lya'])[0]
    ...
    for row in range(256):
        for col in range(64):
            ...
            ftle1_quad[row + offset_row, col + offset_col] = slope

    ftle1[:256, :64] = ftle1_quad.copy()
    ftle1[256:, :64] = ftle1_quad.copy()
    min_diff = np.inf
    origin_row = np.nan
    for row in range(511):
        val_lower = ftle1[row, 63]
        val_upper = ftle1[row + 1, 63]
        if abs(val_lower - val_upper) < min_diff:
            min_diff = abs(val_lower - val_upper)
            origin_row = row
    print(origin_row)

    temp_1 = np.flip(np.flip(ftle1_quad, axis=1), axis=0)
    temp_2 = np.flip(np.flip(ftle1_quad, axis=1), axis=0)

    ftle1[:256, 64:] = np.roll(temp_1, -int((256 - origin_row) / 2), axis = 0)
    ftle1[256:, 64:] = np.roll(temp_2, -int((256 - origin_row) / 2), axis = 0)
...
figname = path + '/ftle_sym.pdf'
plt.savefig(figname, dpi=dpi)
print(figname)

figname = path + '/ftle_sym.png'
plt.savefig(figname, dpi=dpi)
print(figname)